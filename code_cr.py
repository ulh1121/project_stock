...
def relate_radar_weather_data(stock_code):
    ...
    try:
        if arr_list[0][3][0]=='+':
            foreign_ms = int(arr_list[0][3][1:].replace(',',''))
        else:
            foreign_ms= (-1)*int(arr_list[0][3][1:].replace(',',''))
    except TypeError:
        foreign_ms=0.01

    try:
        if arr_list[0][4][0]=='+':
            giguan_ms = int(arr_list[0][4][1:].replace(',',''))
        else:
            giguan_ms=(-1)*int(arr_list[0][4][1:].replace(',',''))
    except TypeError:
        giguan_ms=0.01
...
def stock_predict(code,ptype):
    data = stock.get_market_ohlcv_by_date(fromdate="20220101", todate=datetime.now().strftime('%Y%m%d'), ticker=str(code))
    print(data.head())
    data=data[[ptype]]
    y_train=data
    y_test=data
